...
class Main(Frame):
    ...
    def __init__(self, user):
        ...
        self.master.title("民航管理系统")
        self.frame_top = Frame(self)
        self.frame_top.pack(side="top", fill=X)
        Label(self.frame_top, text="民航管理系统", font=("宋体", 30), fg='black', bg='beige').pack(fill=X)
...
class Home(Frame):
    def __init__(self, master):
        ...
        self.im = Image.open("background.jpg")
...
def personal_flight(window, userName=''):  # 查询， 统计航班信息
    ...
    def no_search(event, label_list=None):
        [tree.delete(item) for item in tree.get_children()]
        ...
        if is_select():
            for elem in tree.selection():
                ...
                def confirm():  # 确认添加事件
                    ...
                    sql2 = "update flight set leftTicket = {} where flightId = {}".format(leftTicket,
                                                                                          tree.item(elem, 'values')[0])
                    sql3 = "update customerUser set point = {} where userName = '{}'".format(new_points, userName)
                    data = [string_time, get_userId(userName), tree.item(elem, 'values')[0],
                            get_companyId(tree.item(elem, 'values')[5]), labels[8].get(), labels[4].get(),
                            labels[6].get(), d_price, time.strftime("%Y/%m/%d", time.localtime())]
                    try:
...
def personal_ticket(window, username):  # 查询个人票务信息
    ...
    def cname_search(event):  # 按顾客姓名查找
        [tree.delete(item) for item in tree.get_children()]
        for row_search in get_ticketData(
                " userId = {} and passagerName like '%{}%'".format(user_id, label_list[1].get())):
            tree.insert('', 'end', values=row_search)
...
def airport_flight(window, userName=''):  # 查询机场航班信息
    ...
    def range_search(event):
        [tree.delete(item) for item in tree.get_children()]
        for new_row in get_airport_flightData(airportId,
                                              " ticketMoney>={} and ticketMoney<={};".format(left.get(), right.get())):
            tree.insert('', 'end', values=new_row)
...
def company_flight(window, userName=''):  # 查询公司航班信息
                ...
                for elem in tree.selection():
                    try:
                        if is_saled(tree.item(elem, 'values')[0]):
                            tkinter.messagebox.showinfo('Failed', '目前有{}位客户预定了机票，请通知所有用户退订后再删除!'.format(
                                is_saled(tree.item(elem, 'values')[0])))
                            return
                        execute_sql("delete from flight where flightId='{}'".format(tree.item(elem, 'values')[0]))
                        tkinter.messagebox.showinfo('Succeed', '删除成功！')
                        show_all()
    ...
    def range_search(event):
        [tree.delete(item) for item in tree.get_children()]
        for new_row in get_flightData2(companyId,
                                       " ticketMoney>={} and ticketMoney<={};".format(left.get(), right.get())):
            tree.insert('', 'end', values=new_row)
...
def company_sale(window, userName):  # 查询公司票务信息
    ...
    def cno_search(event):  # 按顾客编号查找
        [tree.delete(item) for item in tree.get_children()]
        for row_search in get_ticketData(
                " userId = {} and  ticket.companyId = {}".format(label_list[1].get(), companyId)):
            tree.insert('', 'end', values=row_search)

    def cname_search(event):  # 按顾客姓名查找
        [tree.delete(item) for item in tree.get_children()]
        for row_search in get_ticketData(
                " passagerName like '%{}%' and  ticket.companyId = {}".format(label_list[3].get(), companyId)):
            tree.insert('', 'end', values=row_search)

    def pno_search(event):  # 按航班编号查找
        [tree.delete(item) for item in tree.get_children()]
        for row_search in get_ticketData(
                " flightId = {} and  ticket.companyId = {}".format(label_list[5].get(), companyId)):
            tree.insert('', 'end', values=row_search)
...
def add_sale(win):  # 添加订单信息
    ...
    def confirm():
        global string_time
        _companyId, _money = execute_sql(
            "SELECT companyId,ticketMoney FROM flight where flightId={};".format(cmb_c.get())).fetchone()
        ...
        data = (
        string_time, cmb_p.get(), cmb_c.get(), _companyId, labels[5].get(), labels[1].get(), labels[3].get(), money,
...
class FrameLeft(Frame):  # 左侧菜单栏
    ...
    def create(self):
        if self.userType == "customerUser":
            Button(self, text="主      页", bg='#7fffd4', font=('华文琥珀', 16), command=lambda: go_home(Main.flag)).pack(
                fill=X)
    ...
    def add_user():  # 添加用户信息事件
        ...
        def confirm():  # 确认添加事件4
            if choose.get() == "否":
                ...
                data = [labels[1].get(), labels[3].get(), labels[5].get(), labels_vip[1].get(), labels_vip[3].get(),
                        labels_vip[5].get()]
            try:
                execute_sql(sql % tuple(data))  # 字符串格式化
